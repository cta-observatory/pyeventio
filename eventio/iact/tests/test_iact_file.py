...
def test_file_open():
    eventio.File(testfile)


def test_n_events():
    ''' tests if the number of events in the testfile is 1 '''
    with eventio.File(testfile) as f:
...
def test_read_telescopes_2():
    with eventio.File(testfile_two_telescopes) as f:
...
def test_iterating_zstd():
    ...
    with eventio.File(testfile_zstd) as f:
        for i, event in enumerate(f):
            assert isinstance(event, eventio.iact.Event)
            if i > 5:
                break
...
def test_event_with_reuse():
    with eventio.File(testfile_reuse) as f:
